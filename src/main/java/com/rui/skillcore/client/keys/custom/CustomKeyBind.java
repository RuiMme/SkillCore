...
public class CustomKeyBind {
    ...
    public CustomKeyBind(String id, String description, int defaultKeyCode, boolean ctrl, boolean shift, boolean alt) {
        ...
        this.defaultKey = InputConstants.Type.KEYSYM.getOrCreate(defaultKeyCode);
    ...
    // 绑定新按键（同时保存修饰键状态）
    public void setKey(InputConstants.Key key, boolean ctrl, boolean shift, boolean alt) {
        this.currentKey = key;
        this.currentCtrl = ctrl;
        this.currentShift = shift;
        this.currentAlt = alt;
    ...
    public Component getDisplayName() {
        if (this.currentKey == InputConstants.UNKNOWN) {
        ...
        return Component.literal(sb.toString()).append(this.currentKey.getDisplayName());
    }

    // 匹配按键与修饰键输入
    public boolean matches(int keyCode, boolean ctrl, boolean shift, boolean alt) {
        if (this.currentKey == InputConstants.UNKNOWN) return false;
    ...
    public boolean hasConflict() {
        ...
        if (mc.options != null && mc.options.keyMappings != null) {
            for (KeyMapping vanillaBind : mc.options.keyMappings) {
    ...
    public InputConstants.Key getKey() { return currentKey; }
