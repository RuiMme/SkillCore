...
public class SkillScreen extends Screen {
    ...
    protected void init() {
        ...
        for (String category : categories) {
            StringTextComponent tabTitle = new StringTextComponent(SkillManager.getCategoryData(category).title);
...
//        drawTabs(matrixStack, mouseX, mouseY);
     ...
     */
    private void drawNineSliceWindow(MatrixStack ms, int x, int y, int width, int height) {
        int u = 0, v = 0;
        ...
        // 画四条边
        AbstractGui.blit(ms, x + left, y, width - left - right, top, u + left, v, imgW - left - right, top, 256, 256);
        AbstractGui.blit(ms, x + left, y + height - bottom, width - left - right, bottom, u + left, v + imgH - bottom, imgW - left - right, bottom, 256, 256);
        AbstractGui.blit(ms, x, y + top, left, height - top - bottom, u, v + top, left, imgH - top - bottom, 256, 256);
        AbstractGui.blit(ms, x + width - right, y + top, right, height - top - bottom, u + imgW - right, v + top, right, imgH - top - bottom, 256, 256);

        // 画中心区域
        AbstractGui.blit(ms, x + left, y + top, width - left - right, height - top - bottom, u + left, v + top, imgW - left - right, imgH - top - bottom, 256, 256);
    }

    private void drawTabsAndPagination(MatrixStack ms, int mouseX, int mouseY, int guiLeft, int guiTop, int xSize, int ySize) {
