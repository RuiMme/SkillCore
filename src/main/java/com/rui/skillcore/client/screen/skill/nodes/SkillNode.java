...
public class SkillNode {
    ...
    private void initIconCache() {
        ...
        if (item != null && !item.equals(Items.AIR)) {
            ...
            if (this.nbt != null && !this.nbt.trim().isEmpty()) {
                try {
                    CompoundNBT parsedTag = JsonToNBT.parseTag(this.nbt);
    ...
    public void render(MatrixStack ms) {
        ...
        player.getCapability(SkillProvider.SKILL_CAP).ifPresent(cap -> {
            ...
            AbstractGui.blit(ms, this.x - 13, this.y - 13, 0, vOffset, 26, 26, 256, 256);

            long cooldown = getCooldownTime(cap, player.level.getGameTime());
            if (this.isItem) {
                RenderSystem.pushMatrix();
                // 同步 MatrixStack 的动态缩放和平移状态到 OpenGL 渲染管线
                RenderSystem.multMatrix(ms.last().pose());
                // 渲染带有 NBT 的缓存 ItemStack (原版物品渲染器会自动根据 NBT 转换药水颜色或附魔光芒)
                ...
                if (!isParentUnlocked) {
                    ...
                    AbstractGui.fill(ms, this.x - 8, this.y - 8, this.x + 8, this.y + 8, 0xD5101010); // 深黑
                } else if (!isUnlocked || !isActive) {
                    RenderSystem.blendFunc(GlStateManager.SourceFactor.ZERO, GlStateManager.DestFactor.SRC_COLOR);
                    AbstractGui.fill(ms, this.x - 8, this.y - 8, this.x + 8, this.y + 8, 0x85252525); // 暗灰
                }
                ...
                // 动态冷却覆膜表现
                if (cooldown < 16L) {
                    AbstractGui.fill(ms, this.x - 8, (int) (this.y - 8 + cooldown), this.x + 8, this.y + 8, 0x99000000);
                }
                ...
                mc.getTextureManager().bind(this.icon);
                if (cooldown < 16L) {
                    AbstractGui.blit(ms, this.x - 8, this.y - 8, 0, 0, 16, (int) cooldown, 16, 16);
                } else {
                    AbstractGui.blit(ms, this.x - 8, this.y - 8, 0, 0, 16, 16, 16, 16);
                }
            }
