...
public class KeybindSettingsScreen extends Screen {
    ...
    public KeybindSettingsScreen(Screen parentScreen) {
        super(new TextComponent("自定义按键管理"));
    ...
    protected void init() {
        ...
        for (int i = 0; i < CustomKeybindRegistry.CATEGORIES.size(); i++) {
            ...
            Button navBtn = this.addWidget(new Button(10, btnY, leftPanelWidth - 20, 20, new TextComponent(category.name), (btn) -> {
        ...
        this.addWidget(new Button(btn1X, bottomY, btnWidth, 20, new TextComponent("恢复当前页默认"), (button) -> {
            if (this.currentCategory != null) {
        ...
        this.addWidget(new Button(btn2X, bottomY, btnWidth, 20, new TextComponent("完成"), (button) -> {
    ...
    public void render(PoseStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.currentCategory != null) {
            ...
            drawCenteredString(matrixStack, this.font, new TextComponent(categoryTitle), leftPanelWidth + (this.width - leftPanelWidth) / 2, 15, 0xAAAAAA);
        }
