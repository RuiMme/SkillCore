...
public class CustomKeybindList extends ExtendedList<CustomKeybindList.Entry> {
    ...
    public class Entry extends ExtendedList.AbstractListEntry<Entry> {
        ...
        public Entry(CustomKeyBind bind) {
            ...
            this.changeButton = new Button(0, 0, 100, 20, StringTextComponent.EMPTY, (btn) -> {
                CustomKeybindList.this.parent.activeBind = bind;
            });

            this.resetButton = new Button(0, 0, 50, 20, new StringTextComponent("重置"), (btn) -> {
                bind.reset();
        ...
        public void render(MatrixStack matrixStack, int index, int top, int left, int width, int height, int mouseX, int mouseY, boolean isMouseOver, float partialTicks) {
            ...
            // 设置按钮文本
            ITextComponent btnText;
            if (CustomKeybindList.this.parent.activeBind == this.bind) {
                // 如果玩家正在等待输入该按键，显示黄色
                btnText = new StringTextComponent("> ")
                        .append(this.bind.getDisplayName().copy().withStyle(TextFormatting.YELLOW))
                        .append(" <").withStyle(TextFormatting.YELLOW);
            } else {
                // 没在等待输入的情况下：
                if (isConflicting) {
                    // 如果存在冲突，字变红！
                    btnText = this.bind.getDisplayName().copy().withStyle(TextFormatting.RED);
                } else {
                    // 正常状态，显示默认白色字体
