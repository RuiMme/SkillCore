...
public class DynamicSkillScreen extends AbstractSkillScreen {
    ...
    public DynamicSkillScreen(String category, TextComponent title) {
        super(title);
...
//        RenderSystem.pushMatrix();
    ...
    private void renderTooltip(PoseStack ms, LocalPlayer player, SkillNode hoveredNode, int mouseX, int mouseY) {
        if (hoveredNode != null) {
            ...
            player.getCapability(SkillProvider.SKILL_CAP).ifPresent(cap -> {
                ...
                if(isParentUnlocked || cap.hasSkill(hoveredNode.getCategory(), hoveredNode.getId())) {
                    tooltip.add(new TextComponent("§e§l" + hoveredNode.getName()));
...
//                    tooltip.add(new StringTextComponent(rawDescription));
                    ...
                    for (SkillNode parent : hoveredNode.getParent()) {
                        ...
                        tooltip.add(new TextComponent("  " + icon + " §8" + parent.getName()));
                    }
                    ...
                    if (!cap.hasSkill(hoveredNode.getCategory(), hoveredNode.getId())) {
                        ...
                        tooltip.add(new TextComponent(lvlColor + "• 需要等级: " + lvl));
                        ...
                        hoveredNode.getInfo().getCost().forEach((item, count) -> {
                            ...
                            tooltip.add(new TextComponent(itemColor + "• 需要 " + item.getName(ItemStack.EMPTY).getString() + " x" + count));
                        });
                    } else {
                        tooltip.add(new TextComponent("§b§o[已激活]"));
                        if (cap.isActive(hoveredNode.getCategory(), hoveredNode.getId())) {
                            tooltip.add(new TextComponent("§a状态: 已开启"));
                        } else {
                            tooltip.add(new TextComponent("§c状态: 已关闭"));
                        }
                        if(hoveredNode.getCooldownTime(cap, player.level.getGameTime()) != 16L) {
                            tooltip.add(new TextComponent("§c技能冷却中"));
                        }
                    }
                } else {
                    tooltip.add(new TextComponent("§e§l???"));
                    Prerequisites(hoveredNode, tooltip);
                    for (SkillNode parent : hoveredNode.getParent()) {
                        boolean unlocked = cap.hasSkill(parent.getCategory(), parent.getId());
                        String icon = unlocked ? "§a✔" : "§c✘";
                        if(!unlocked) {
                            tooltip.add(new TextComponent("  " + icon + " §8???"));
                        } else {
                            tooltip.add(new TextComponent("  " + icon + " §8" + parent.getName()));
                        }
                    }
    ...
    private void Prerequisites(SkillNode hoveredNode, List<Component> tooltip) {
        tooltip.add(new TextComponent("§8" + "--------------------"));

        if (!hoveredNode.getParent().isEmpty()) {
            tooltip.add(new TextComponent("§7前置要求: "));
        }
    }

    // --- 动态视口背景填充算法 ---
    private void renderScrollingBackground(PoseStack ms) {
//        this.minecraft.getTextureManager().bind(new ResourceLocation(SkillManager.getCategoryData(this.category).background));
    ...
    // 连线
    private void drawConnector(PoseStack ms, LocalPlayer player, SkillNode parent, SkillNode child) {
